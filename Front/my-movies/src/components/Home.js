...
class Home extends Component {
  ...
  onEnter = (e) => {
    ...
    if(e.key == 'Enter'){
      if(this.state.searchValue.length > 1 && this.state.searchValue != this.state.tempSearchValue){
        this.fetchData(url);
  ...
  render() {
    ...
    function manageHomeDisplay(){
      if(searchValue.length > 0 && searchData.length == undefined && searchData != null){
        return <FilmList title="Résultat de la recherche" datas={searchData.results} dataLoaded={true}/>
      } else {
        return <section>
